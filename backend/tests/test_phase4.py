"""
Integration tests — hits every endpoint, checks the full pipeline.
...
class TestDwightAPI:
    ...
    def test_chat_services(self):
        ...
        r = requests.post(
            f"{BASE_URL}/api/chat",
            json={"message": "What services does GTMT offer?"},
            timeout=180
        )
        assert r.status_code == 200
        data = r.json()
